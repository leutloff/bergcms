/*
 * echo.cpp
 * based on the echo example from
 * http://cgi.sourceforge.net/docs/fastcgi___cgi/examples/cgi_examples.html
 */
...
void format_map(OStream& os, Request& req, Map& m, const std::string& title)
{
    ...
    if (m.empty()) os << "<div class=\"var_pair\">EMPTY</div>";
    else
    {
        for (typename Map::const_iterator iter(m.begin()), end(m.end()); iter != m.end(); ++iter)
        {
            os << "<div class=\"var_pair\">"
               "<div class=\"var_name\">"
               << iter->first << "</div>"
               "<div class=\"var_value\">"
               << iter->second << (req.uploads.count(iter->first) ? " (file)" : "") << "</div>"
               "</div>";
        }
    }
    os << "</div>";
